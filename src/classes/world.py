...
class World:
    ...
    def screen_to_tile(self, x: int, y: int) -> tuple[int, int]:
        ...
        return int(x / C.WORLD_SCALE) // int(self.map.tile_width), int(y / C.WORLD_SCALE) // int(self.map.tile_height)

    def tile_to_screen(self, tile_x: int, tile_y: int) -> tuple[int, int]:
        return int((tile_x * self.map.tile_width + self.map.tile_width // 2) * C.WORLD_SCALE),\
            int((tile_y * self.map.tile_height + self.map.tile_height // 2) * C.WORLD_SCALE)
        ...
        while heap:
            ...
            for i in range(-1, 2):
                for j in range(-1, 2):
                    ...
                    total_cost = lengths[current_position] + (abs(x - destination_tile[0]) + abs(y - destination_tile[1]))
